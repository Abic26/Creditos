...
function calcular(){
    ...
    setLocalStorage(nombre,valCre,cuota,totalP,totalU,creSel)
    }
    function setLocalStorage(nombre,valCre,cuota,totalP,totalU,creSel){
        const mensaje = alert("Se ingresaron a la base de datos nuevos elementos")
        arrayElemt.push({arrayNombre: nombre, arrayValc: valCre, arrayCuo: cuota, arraP: totalP, arrayU: totalU, arrayCreS: creSel})
        localStorage.setItem("arrayElemt",JSON.stringify(arrayElemt))
        document.getElementById("mensaje")
    }
        
    // Codigo suaves de abic
// let arrayElemt = JSON.parse(localStorage.getItem('arrayElemt')) || [];
// function calcular(){
// let nombre = document.getElementById("nombre").value;
// let cuota = parseInt(document.getElementById("cuota").value);
